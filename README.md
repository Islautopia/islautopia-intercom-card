...
* **No door button when there's no door:** if the doorbell has no lock configured, the open button isn't drawn at all instead of being offered and failing. The card learns this from the doorbell's own `no_lock_configured` answer and remembers it per device, so the failed attempt happens once rather than on every page load — and a later successful open makes it reappear.
